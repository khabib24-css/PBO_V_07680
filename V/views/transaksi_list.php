                ...
                <tbody class="text-gray-700">
                    <?php if (!empty($transaksis)) {
                        foreach ($transaksis as $transaksi) { 
                            if (is_null($transaksi->customer) || is_null($transaksi->kasir) || is_null($transaksi->barangs)) {
                                continue;
                            }
                            ?>
                    <tr class="text-center">
                        <td class="py-3 px-4 text-blue-600"><?php echo htmlspecialchars($transaksi->idTransaksi ?? ''); ?></td>
                        <td class="w-1/4 py-3 px-4"><?php echo htmlspecialchars($transaksi->customer->username ?? ''); ?></td>
                ...
                <div class="mt-3 text-center">
                    ...
                    <div class="mt-2">
                        <table class="min-w-full bg-white">
                            ...
                            <tbody class="text-gray-700">
                                <?php if (!empty($transaksi->barangs)) {
                                    foreach ($transaksi->barangs as $index => $barang) { ?>
                                    <tr class="text-center">
                                        <td class="py-3 px-4"><?php echo htmlspecialchars($barang->item_name ?? ''); ?></td>
